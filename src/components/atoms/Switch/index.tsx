...
type SwitchType<T extends string> = {
 ...
 function Switch<T extends string>({
  ...
  setStateValue,
    ...
    const switchDynamicStyle = `bg-oceanBlue translate-x-[25px] border-green`;
    const isFirstValue = stateValue === values[0];

    const handleKeyDown =(e: KeyboardType) => {
        if(keyOptions[e.code])  {
          const handleValue = isFirstValue ? values[0]: values[1];
...
<div className='flex w-20' aria-hidden="true">
  <span onClick={()=> setStateValue(values[0])} className={`cursor-pointer ${isFirstValue ? "inline ": "hidden"} ${hoverStyle}`} aria-hidden="true">{values[0]}</span>
  <span onClick={()=> setStateValue(values[1])} className={`cursor-pointer ${isFirstValue ? "hidden": "inline"} ${hoverStyle}`} aria-hidden="true">{values[1]}</span>
</div>
</div>
  );
}
