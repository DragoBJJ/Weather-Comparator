...
export const Input = memo<InputProps>(({ type,value, border, placeholder, size, setValue }) => {
  return (
    <input
      ...
      className={`${InputSizeVariants[size]} ${border ? InputBorderVariants[border]: ""} mt-4 placeholder-gray-700 text-black placeholder-opacity-75 outline-none border-[1px] transition-all ease-in-out duration-100 border-cashmere  content-center hover:cursor-pointer pl-4  text-md shadow-md tracking-wider  rounded-3xl  `}
    />
  );
});
